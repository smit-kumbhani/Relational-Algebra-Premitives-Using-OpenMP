...
int main() {
    ...
    vector<string> file_name;vector<int> sizeofmat;
    file_name.pb("Tina_AskCal.csv");   sizeofmat.pb(11);
    file_name.pb("GD02_a.csv");        sizeofmat.pb(23);
    file_name.pb("ibm32.csv");         sizeofmat.pb(32);
    file_name.pb("GD95_c.csv");        sizeofmat.pb(62);
    file_name.pb("GD02_b.csv");        sizeofmat.pb(80);
    file_name.pb("GD97_a.csv");        sizeofmat.pb(84);
    file_name.pb("GD98_b.csv");        sizeofmat.pb(121);
    file_name.pb("GD96_d.csv");        sizeofmat.pb(180);
    file_name.pb("Harvard500.csv");    sizeofmat.pb(500);
    file_name.pb("Roget.csv");         sizeofmat.pb(1022);
    file_name.pb("GD06_Java.csv");     sizeofmat.pb(1538);
    file_name.pb("CSphd.csv");         sizeofmat.pb(1882);
    file_name.pb("Kohonen.csv");       sizeofmat.pb(4470);
    file_name.pb("EPA.csv");           sizeofmat.pb(4772);
       ...
       for(int p0=0;p0<vertices.size();p0++)        //no.of vertices
            {
                ...
                for(itr = umap2[vertices[p0]].begin();itr != umap2[vertices[p0]].end();itr++)    //length of adjacency list -> no.of vertices
                {
                    ...
                    {
                        if(umap2[vertices[p0]].count(*itr2)==0)
                        {
...
{
    //cout << "-----file name = " << file_name[itr] << endl;
    //cout << "for processors = " << procs[proc_id] << endl;
    tot_time = 0;
    ...
    {  
        ...
        {
            //parallelize from here
            f(i,nthreads) checker[i] = false;

            for(int p0=id;p0<vertices.size();p0+=nthreads)        //no.of vertices
            {
                set<int>::iterator itr;
                for(itr = umap2[vertices[p0]].begin();itr != umap2[vertices[p0]].end();itr++)    //length of adjacency list -> no.of vertices
                {
            ...
            bool fin = false;
            f(i,nthreads) fin = fin || checker[i]; 
            if(!fin) break;
        }
